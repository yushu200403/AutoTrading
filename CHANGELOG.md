...
- 移除待对账导致的模型停机与后台循环退出，启动校时失败进入后台重试；原有亏损、回撤、用量与连续失败熔断仍保持移除。
- 新增逐周期订单对账、保护单补建和数量同步、残留保护单清理；修复请求先持久化，未知结果查询原订单，避免重复发送。
- 前序工具失败后重新校验实际账户状态并继续处理后续动作；历史异常和失败详情反馈模型，只隔离可能与未确认订单冲突的动作。
- 持久化客户端订单 ID、执行错误、修复步骤和批量撤单目标；新增可回滚迁移 `20260907_04`，修复完成状态为 `RECONCILED`。
- 保护单创建超时不再误报补偿完成，也不盲目反向回补；新增模拟账本和重启恢复回归测试。
