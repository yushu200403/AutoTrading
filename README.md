...
控制台的行情、余额、持仓、净值、交易记录和模型推理等业务数据均允许访客直接查看；除设置页外，未登录与已登录的数据展示保持一致。未登录时不显示设置页；输入控制台密码后才会显示设置，并允许执行启动、停止、单次运行、一键全平、实盘切换和保存指令等操作。服务端仍会校验会话与 CSRF，隐藏设置页不是唯一安全边界。若将站点公开到互联网，请确认可以接受上述业务数据公开。
