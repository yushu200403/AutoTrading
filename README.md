...
- 账户级熔断：日亏损、峰值回撤和模型 token 预算超限时暂停交易循环并要求人工处理。
- 可追溯性：市场快照、交易意图（保留模型原始输出）、执行结果、周期状态、token 用量和分模式净值历史。
- 控制台：服务端 Session、CSRF、CSP 与防嵌套等响应头，只读与控制接口均可配置认证。
...
| `RISK_MAX_DAILY_LOSS_PERCENT` | 10 | 24 小时净值回落上限，触发后暂停交易，0 表示关闭 |
| `RISK_MAX_DRAWDOWN_PERCENT` | 25 | 相对历史峰值净值的回撤上限，0 表示关闭 |
| `AI_MAX_TOOL_CALLS` | 10 | 单周期最大工具调用数 |
| `AI_MAX_DAILY_TOKENS` | 0 | 24 小时模型 token 预算，0 表示不限制 |
| `MAX_CONSECUTIVE_CYCLE_FAILURES` | 5 | 连续未成功周期数上限，达到后暂停循环 |
...
- 单个周期内部异常不会终止后台循环；连续失败会指数退避，达到配置上限后暂停并记录原因。
