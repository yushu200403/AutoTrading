...
- 控制台：访客可直接查看全部账户与交易数据；未登录时隐藏设置页，控制与修改接口使用服务端 Session、CSRF 和密码认证，并提供 CSP 与防嵌套等响应头。
...
控制台的行情、余额、持仓、净值、交易记录和模型推理等业务数据默认允许访客直接查看；除设置页外，未登录与已登录的数据展示保持一致。若设置 `CONSOLE_READONLY_AUTH_ENABLED=true`，访客只能看到顶部登录提示，登录后才能读取业务数据。未登录时不显示设置页；输入控制台密码后才会显示设置，并允许执行启动、停止、单次运行、一键全平、实盘切换和保存指令等操作。服务端仍会校验会话与 CSRF，隐藏设置页和数据区不是唯一安全边界。若将站点公开到互联网，请确认可以接受当前配置对应的数据可见范围。
...
- 工具执行失败不会触发模型重发已执行交易。
- 前序失败后按最新账户状态重新校验后续动作，不再整体跳过后续交易；失败详情反馈给下一周期模型调整。
- 市价单超时后按客户端订单 ID 对账；无法确认时返回 `UNKNOWN`，禁止自动重试。
- 撤单遇到网络异常同样返回 `UNKNOWN`，不会改用另一套订单 ID 空间重试。
- 新增仓位的保护单被明确拒绝时自动反向回补；保护单结果未知时保留订单身份等待查询，补偿失败标记为 `CRITICAL` 并进入自动修复。
- `PENDING`、`UNKNOWN`、`PARTIAL`、`CRITICAL` 决策由每周期自动对账处理，不会停止模型调用；已确认成交但保护缺失时按最新仓位修复，仓位已平时清理残留保护单。
- 修复请求先落盘再发送；结果未知时按原身份继续查询，不重放市价交易或未确认的保护单创建。修复完成记录为 `RECONCILED`，保留原状态、错误和修复进度供审计。
- 无法确认的订单只限制相关交易对可能重复或冲突的动作；其他交易对及记忆更新继续处理。旧保护价失效时反馈给模型重新设价，批量撤单恢复仅处理当时记录的目标订单。
- 交易所杠杆信息无法确认时不会按 1 倍处理，而是直接拒绝开仓。
- 单个周期内部异常或失败结果不会终止后台循环，也不会扩大调用间隔；下一周期仍按 `TRADING_INTERVAL_MINUTES` 调度。
- 自动循环不再因业务错误、待对账、亏损、回撤、token 用量或连续失败自动暂停；用户仍可主动停止。校时失败进入后台重试，恢复摘要通过状态接口的 `recovery` 返回。
...
│   │   ├── recovery.py       # 订单对账、保护单修复与进度持久化
│   │   ├── risk.py           # 配置化执行前风控
│   │   ├── service.py        # 后台持续循环生命周期与校时恢复
...
- 外部订单后若审计回写中断，后续周期自动查询原订单。缺失订单身份、历史批量撤单清单或交易所查询证据时，该记录仍会保持待修复并反馈模型；不会把查询失败当作订单未成交，也不能保证所有外部故障都能自动解决。
- 升级到本次自动恢复版本时，先运行 `flask db upgrade` 应用 `20260907_04` 迁移，增加订单身份、错误原因和修复进度字段；进程退出、主机断网或服务宕机期间无法保证模型调用成功。
